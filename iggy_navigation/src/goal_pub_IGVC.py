...
class Navigator():
    ...
    def makeWaypointsIntoGoals(self, filename):
        ...
        restDur=0.0
        goalstemp=[]
        xOffSet = rospy.set_param("xOffSet", 0.0) #lattitude aka east-west
        yOffSet = rospy.set_param("yOffSet", 0.0) #longitude aka north - south
        ...
        for waypointLat,waypointLong,search_duration,rest_duration,unused,identity in wps:
            ...
            if(identity[0]!="a"):
                goal_pose.position.x=goal_pose.position.x -xOffSet # REP103 says x is east and y is north
                goal_pose.position.y= goal_pose.position.y - yOffSet
